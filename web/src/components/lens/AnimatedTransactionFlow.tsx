...
export function AnimatedTransactionFlow({ activeCardId, txData }: Props) {
  const [expandedList, setExpandedList] = useState<'inputs' | 'outputs' | null>(null);
  ...
  const visibleOutputsCount = showOutputSummary ? MAX_VISIBLE_NODES + 1 : totalOutputs;
    ...
    <div className="w-full h-full min-h-125 flex items-center justify-center bg-black rounded-2xl border border-surface-border overflow-hidden relative">
      ...
      <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full h-full drop-shadow-2xl" style={{ filter: 'drop-shadow(0 20px 30px rgba(0,0,0,0.5))' }}>
        ...
        {Array.from({ length: visibleOutputsCount }).map((_, i) => {
          ...
          return (
            <g key={`path-out-${i}`}>
              ...
              {activePath && (
                <circle r="3" fill={coreColor} style={{ filter: `drop-shadow(0 0 4px ${coreColor})` }}>
                  <animateMotion dur={`${2 + i * 0.1}s`} repeatCount="indefinite" path={pathD} />
                </circle>
              )}
            </g>
          );
        ...
        {(() => {
          ...
          const strokeCol = isFeeHighlighted ? "#ef4444" : "var(--color-surface-border)"; // red-500 or border
          ...
          return (
            <g key={`path-fee`}>
              <path
                ...
                style={isFeeHighlighted ? { filter: "drop-shadow(0 0 6px #ef4444)" } : undefined}
              />
              {activePath && (
        ...
        >
          ...
          <circle 
            cx={centerX} cy={centerY} r={coreRadius - 8} 
          ...
          <line x1={centerX + coreRadius - 4} y1={centerY} x2={centerX + coreRadius + 4} y2={centerY} stroke={coreColor} strokeWidth="2" opacity="0.6" />
          <line x1={centerX} y1={centerY - coreRadius - 4} x2={centerX} y2={centerY - coreRadius + 4} fill="none" stroke={coreColor} strokeWidth="2" opacity="0.6" />
          <line x1={centerX} y1={centerY + coreRadius - 4} x2={centerX} y2={centerY + coreRadius + 4} fill="none" stroke={coreColor} strokeWidth="2" opacity="0.6" />
          ...
          {!isWarningCard && (
            <g transform={`translate(${centerX - 12}, ${centerY - 12})`} className="text-lens-400">
        ...
        {txData.vin.slice(0, MAX_VISIBLE_NODES).map((vin, i) => {
          ...
          const displayAmount = vin.prevout?.value_sats !== undefined ? (vin.prevout.value_sats / 100_000_000).toFixed(4) : "Unknown";
          ...
          return (
            ...
            >
              <rect
                ...
                strokeWidth="1"
                style={{ backdropFilter: 'blur(4px)', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.5))' }}
              />
              <rect x={leftX} y={y} width="3" height={nodeH} fill="#10b981" rx="1.5" style={{ filter: "drop-shadow(0 0 6px #10b981)" }} />
              
              <text x={leftX + 16} y={y + 24} fill="#ffffff" fontSize="13" fontFamily="sans-serif" fontWeight="bold" style={{ filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.8))" }}>
                {displayAmount} <tspan fill="#94a3b8" fontSize="11" fontWeight="normal">BTC</tspan>
              </text>
              <text x={leftX + 16} y={y + 44} fill="#64748b" fontSize="10" fontFamily="monospace">
                {vin.txid.slice(0, 16)}...
              </text>
              <text x={leftX + 16} y={y + 56} fill="#4b5563" fontSize="9" fontFamily="sans-serif" className="uppercase tracking-wider hover:fill-lens-400 hover:cursor-help transition-colors">
        ...
        {showInputSummary && (() => {
          ...
          return (
            ...
            >
              <rect x={leftX} y={y} width={nodeW} height={nodeH} rx="8"
          ...
          return (
            ...
            >
              <rect
                ...
                style={{ backdropFilter: 'blur(4px)', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.5))' }}
              />
              <rect x={rightX + nodeW - 3} y={y} width="3" height={nodeH} fill="#8b5cf6" rx="1.5" style={{ filter: "drop-shadow(0 0 6px #8b5cf6)" }} />
              
              <text x={rightX + 16} y={y + 24} fill="#ffffff" fontSize="13" fontFamily="sans-serif" fontWeight="bold" style={{ filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.8))" }}>
                {(vout.value_sats / 100_000_000).toFixed(4)} <tspan fill="#94a3b8" fontSize="11" fontWeight="normal">BTC</tspan>
              </text>
              <text x={rightX + 16} y={y + 44} fill="#64748b" fontSize="10" fontFamily="monospace">
                {vout.address ? vout.address.slice(0, 20) + '...' : 'OP_RETURN / Unknown'}
              </text>
              <text x={rightX + 16} y={y + 56} fill="#4b5563" fontSize="9" fontFamily="sans-serif" className="uppercase tracking-wider hover:fill-lens-400 hover:cursor-help transition-colors">
        ...
        {showOutputSummary && (() => {
          ...
          return (
            <motion.g
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: isOutputDimmed || isWarningCard ? 0.3 : 1, x: 0 }}
              transition={{ delay: 0.5, duration: 0.8 }}
              className="cursor-pointer transition-opacity duration-500"
              onClick={() => setExpandedList('outputs')}
            >
              <rect x={rightX} y={y} width={nodeW} height={nodeH} rx="8"
                fill="#18181b" stroke="#3f3f46" strokeWidth="1"
                className="hover:stroke-lens-500 transition-colors duration-300"
              />
          ...
          return (
            ...
            >
              <rect
                ...
                width={nodeW} height={nodeH}
                rx="6"
                fill={isFeeHighlighted ? "rgba(239, 68, 68, 0.1)" : "url(#glassNode)"}
                stroke={isFeeHighlighted ? "#ef4444" : "rgba(244, 63, 94, 0.4)"}
                strokeWidth={isFeeHighlighted ? "2" : "1"}
                className="transition-all duration-500"
                style={{ backdropFilter: 'blur(4px)', filter: isFeeHighlighted ? "url(#glow-red)" : "drop-shadow(0 4px 6px rgba(0,0,0,0.5))" }}
              />
              <rect x={rightX + nodeW - 3} y={y} width="3" height={nodeH} fill={isFeeHighlighted ? "#ef4444" : "#f43f5e"} rx="1.5" className="transition-colors duration-500" style={{ filter: isFeeHighlighted ? "drop-shadow(0 0 8px #ef4444)" : "drop-shadow(0 0 4px #f43f5e)" }} />
              
              <text x={rightX + 16} y={y + 24} fill={isFeeHighlighted ? "#fbbf24" : "#ffffff"} fontSize="13" fontFamily="sans-serif" fontWeight="bold" className="transition-colors duration-500" style={{ filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.8))" }}>
                {(txData.fee_sats / 100_000_000).toFixed(8)} <tspan fill="#ef4444" fontSize="11" fontWeight="normal">BTC</tspan>
              </text>
              <text x={rightX + 16} y={y + 42} fill={isFeeHighlighted ? "#fca5a5" : "#64748b"} fontSize="11" fontFamily="sans-serif" fontWeight="bold" className="transition-colors duration-500 hover:fill-red-400 hover:cursor-help">
                <title>The difference between the total inputs and total outputs. This is exactly what the miner gets paid for including your transaction in the block.</title>
                Miner Fee
              </text>
              <text x={rightX + 16} y={y + 56} fill="#4b5563" fontSize="9" fontFamily="sans-serif" className="hover:fill-zinc-400 hover:cursor-help transition-colors">
                <title>Weight Units (WU): How much physical hard drive space this transaction takes up on the blockchain.</title>
                Weight: {txData.weight} WU
              </text>
              {isFeeHighlighted && (
                <g transform={`translate(${rightX + nodeW - 32}, ${y + 16})`} className="text-red-500">
                  <FlameIcon />
                </g>
              )}
            </motion.g>
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-surface-border bg-black/40">
                <div>
                  <h3 className="text-xl font-bold text-white">
                    All {expandedList === 'inputs' ? totalInputs : totalOutputs} {expandedList === 'inputs' ? 'Inputs' : 'Outputs'}
                  </h3>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-black/20">
                <div className="flex flex-col gap-3">
                  {(expandedList === 'inputs' ? txData.vin : txData.vout).map((item, idx) => {
                    const valueSats = 'prevout' in item ? (item.prevout?.value_sats || 0) : item.value_sats;
                      ...
                      <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-white/5 bg-white/5 hover:bg-white/10 transition-colors">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-mono text-zinc-300 truncate">
                            {txid ? `${txid.slice(0,32)}...` : (address || 'OP_RETURN / Data Payload')}
                          </p>
                        </div>
                        <div className="text-left sm:text-right shrink-0">
                          <p className="text-sm font-bold text-white tracking-tight">
                            {(valueSats / 100_000_000).toFixed(8)} <span className="text-zinc-500 text-xs">BTC</span>
                          </p>
                        </div>
                      </div>
                    );
