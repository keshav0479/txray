...
export function AnimatedPsbtFlow({
  activeCardId,
  data,
  onSummaryClick,
}: Props) {
  ...
  if (!showOutputSummary) {
    ...
    if (
      changeIdx !== null &&
      changeIdx >= firstN &&
      !visibleOutputIndices.includes(changeIdx)
    ) {
  ...
  const visibleOutputsCount = showOutputSummary
    ? visibleOutputIndices.length + 1
    : visibleOutputIndices.length; // +1 for summary node
  ...
  return (
    <div className="w-full h-full min-h-125 flex items-center justify-center bg-black rounded-2xl border border-surface-border overflow-hidden relative">
      ...
      >
            ...
            return (
              <g key="po-summary">
                <path
                  ...
                  stroke="var(--color-surface-border)"
                  strokeWidth="2"
                  opacity={isOutputDimmed ? 0.15 : 0.5}
                  className="transition-opacity duration-500"
                />
                {active && (
                  <circle
        ...
        {/* === FEE PATH === */}
        {(() => {
          const feeSlotIdx = showOutputSummary
            ? visibleOutputIndices.length + 1
            : visibleOutputIndices.length;
        ...
        >
          ...
          <circle
            ...
            strokeWidth="1.5"
            className="transition-colors duration-500"
            style={{ filter: `drop-shadow(0 0 10px ${coreColor}40)` }}
          />
          <circle
            cx={centerX}
            cy={centerY}
        ...
        {data.selected_inputs.slice(0, MAX_VISIBLE_NODES).map((input, i) => {
          ...
          const opacity =
            isWarningCard || isSettingsCard || isInputDimmed ? 0.15 : 1;
          return (
            <motion.g
              key={`in-${i}`}
        ...
        {showInputSummary &&
          (() => {
            ...
            const hiddenInputSats = data.selected_inputs
              .slice(MAX_VISIBLE_NODES)
              .reduce((s, inp) => s + inp.value_sats, 0);
            return (
              <motion.g
                initial={{ opacity: 0, x: -20 }}
                animate={{
        ...
        {visibleOutputIndices.map((outputIdx, slotIdx) => {
          ...
          const opacity =
            isOutputDimmed || isWarningCard || isSettingsCard ? 0.15 : 1;
          ...
          return (
            <motion.g
              ...
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity, x: 0 }}
              transition={{ duration: 0.5, delay: slotIdx * 0.1 }}
              className="transition-opacity duration-500"
            >
              <rect
                x={rightX}
                ...
                fill="url(#glassNode)"
                stroke={
                  isChange ? "rgba(16,185,129,0.5)" : "rgba(139,92,246,0.4)"
                }
                strokeWidth="1"
              />
              <rect
                ...
                width="3"
                height={nodeH}
                fill={isChange ? "#10b981" : "#8b5cf6"}
                rx="1.5"
                style={{
                  filter: `drop-shadow(0 0 6px ${isChange ? "#10b981" : "#8b5cf6"})`,
                }}
              />
              <text
                x={rightX + 16}
        ...
        {showOutputSummary &&
          (() => {
            ...
            const hiddenSats = data.outputs.reduce(
              (s, o, i) =>
                visibleOutputIndices.includes(i) ? s : s + o.value_sats,
              0,
            );
            return (
              <motion.g
                ...
                animate={{
                  opacity:
                    isOutputDimmed || isWarningCard || isSettingsCard ? 0.3 : 1,
                  x: 0,
                }}
                transition={{ delay: 0.5, duration: 0.8 }}
                className="transition-opacity duration-500 cursor-pointer"
                onClick={() => onSummaryClick?.("outputs")}
              >
                <rect
                  x={rightX}
                  y={y}
        ...
        {(() => {
          ...
          const opacity =
            isFeeDimmed || isWarningCard || isSettingsCard ? 0.3 : 1;
          return (
            <motion.g
              key="fee"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity, x: 0 }}
              transition={{ duration: 0.5, delay: feeSlotIdx * 0.1 }}
              className="transition-all duration-500"
            >
              <rect
                x={rightX}
                ...
                rx="6"
                fill={
                  isFeeHighlighted ? "rgba(239,68,68,0.1)" : "url(#glassNode)"
                }
              ...
              >
                <title>
                  The difference between total inputs and total outputs. This is
                  what the miner earns for including your transaction.
                </title>
              ...
              >
                <title>
                  Weight Units (WU): How much block space this transaction
                  occupies.
                </title>
